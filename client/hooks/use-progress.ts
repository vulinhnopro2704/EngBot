...
export function useProgress() {
  const { streak, hearts, diamonds, dailyGoal, dailyProgress, progress, updateProgress } = useVocabStore()

  const calculateCourseProgress = (courseId: number) => {
    const coursePrefix = `${courseId}-`
    const courseLessons = Object.keys(progress).filter((key) => key.startsWith(coursePrefix))

    if (courseLessons.length === 0) return 0

    const totalProgress = courseLessons.reduce((sum, key) => sum + progress[key], 0)
  ...
  const recordActivity = (courseId: number, lessonId: number, activityType: string, points: number) => {
