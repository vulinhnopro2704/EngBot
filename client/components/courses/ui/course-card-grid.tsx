      ...
      <Card className="h-full overflow-hidden border-none shadow-md">
        <div className="relative">
          <div className="h-32 xs:h-36 sm:h-40 md:h-48 bg-gradient-to-r from-blue-500/20 to-purple-500/20 flex items-center justify-center overflow-hidden">
            {course.imageUrl ? (
              ...
              <Icon className="h-12 w-12 xs:h-14 xs:w-14 sm:h-16 sm:w-16 md:h-20 md:w-20 text-primary/40" />
            )}
          </div>
        ...
        <CardHeader className="pb-1 sm:pb-2 p-3 sm:p-4">
          <div className="flex justify-between items-start">
            <div className={`p-1.5 sm:p-2 rounded-lg ${course.color}`}>
              <Icon className={`h-3.5 w-3.5 xs:h-4 xs:w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 ${course.iconColor}`} />
            </div>
          </div>
        ...
        <CardContent className="space-y-3 sm:space-y-4 p-3 sm:p-4 pt-0 sm:pt-0">
          <div className="space-y-1 sm:space-y-2">
            ...
            <Progress value={progress} className="h-1.5 xs:h-2 md:h-3" />
          </div>
